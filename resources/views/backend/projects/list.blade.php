                            ...
                            <table id="datatable" class="table table-bordered dt-responsive nowrap"
                                ...
                                @foreach($projects as $row=>$project)
                                    <tr>
                                        ...
                                        <td class="text-center">{{ucwords($project->name)}}</td>
                                        <td class="text-center">{{ucwords($project->client->name)}}</td>

                                        <td class="text-center">{{ucwords($project->service->name)}}</td>
                                        ...
                                        <td class="text-center">
                                            @if($project->status == 0)
                                                <a type="button"
                                                   href="{{route('project.status.update',[$project->id,1])}}"
                                                   class="badge badge-danger-inverse">Incomplete</a>
                                            @elseif($project->status == 1)
                                                <a type="button"
                                                   href="{{route('project.status.update',[$project->id,0])}}"
                                            ...
                                            @endif
                                            <br>
                                            @if($project->milestone->count())
                                                <div class="mt-2">
                                                    Milestone(s): {{$project->milestone->count()}}
                                                </div>
                                            @endif
                                        </td>

                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link" id="profile-07-tab" data-toggle="tab" href="#profile-07"
                                           role="tab" aria-controls="profile-07" aria-selected="false"><i
                                                class="fa fa-user"></i> Milestones </a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" id="portfolio-07-tab" data-toggle="tab" href="#portfolio-07"
                                           role="tab" aria-controls="portfolio-07" aria-selected="false"><i
                                                class="fa fa-picture-o"></i> Tasks </a>
                                    </li>
                                    <li class="nav-item">
                                    ...
                                    <div class="tab-pane fade py-3 active show" id="home-07" role="tabpanel"
                                        ...
                                        <form action="{{route('project.update',$project->id)}}" method="POST">
                                            ...
                                            <div class="form-group">
                                                ...
                                                @if($errors->has('name'))
                                                    <div
                                                        class="invalid-feedback d-block">{{$errors->first('name')}}</div>
                                                @endif
                                            </div>
                                                ...
                                                <select class="js-basic-single form-control" name="client_name">
                                                    @foreach($clients as $client)
                                                        <option value="{{$client->id}}"
                                                                @if($client->id == $project->client_id) selected @endif>{{$client->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                                ...
                                                <select class="js-basic-single form-control" name="service_name">
                                                    @foreach($services as $service)
                                                        <option value="{{$service->id}}"
                                                                @if($service->id == $project->type) selected @endif>{{$service->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="quotation" class="form-control"
                                                       placeholder="Enter Quotation" value="{{$project->quotation}}">
                                                @if($errors->has('quotation'))
                                                    <div
                                                        class="invalid-feedback d-block">{{$errors->first('quotation')}}</div>
                                                @endif
                                            </div>
                                                ...
                                                <div class=" card-statistics">
                                                    <div class="card-header container-fluid">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-4">
                                                                <a href="{{route('project.milestone.create',[$project->id])}}"
                                                    ...
                                                    <div class="card-body">
                                                        <table id="datatable"
                                                            ...
                                                            <tbody>
                                                            @foreach($milestones as $row=>$milestone)
                                                                <tr>
                                                                    <td class="text-center">{{++$row}}</td>
                                                                    <td class="text-center">{{ucwords($milestone->name)}}</td>


                                                                    <td class="text-center">{{Str::limit($milestone->description,30)}}</td>

                                                                    <td class="text-center">{{$milestone->task->count()}}</td>

                                                                    <td class="text-center">
                                                                        @if($project->status == 0)
                                                                            <a type="button"
                                                                               href="{{route('project.status.update',[$milestone->id,1])}}"
                                                                               class="badge badge-danger-inverse">Incomplete</a>
                                                                        @elseif($project->status == 1)
                                                                            <a type="button"
                                                                               href="{{route('project.status.update',[$milestone->id,0])}}"
                                                                    ...
                                                                    <td class="text-center">
                                                                        {{date('F d, Y',strtotime($milestone->start_date))}}
                                                                        <br>
                                                                        to
                                                                        <br>
                                                                        {{date('F d, Y',strtotime($milestone->completion_date))}}
                                                                    </td>
                                                                    <td class="text-center">
                                                                        <a href="{{route('project.milestone.edit',[$project->id,$milestone->id])}}"
                                                                           class="btn btn-round btn-inverse-success" type="button">edit</a>
                                                                        <a href="{{route('project.milestone.view',[$project->id,$milestone->id])}}"
                                                                           class="btn btn-round btn-inverse-primary" type="button">view</a>
                                                                        <a href="{{route('project.milestone.delete',[$project->id,$milestone->id])}}"

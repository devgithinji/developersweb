                    ...
                    <div class="card card-statistics p-3">
                        <div class="card-header">
                            <div class="card-heading">
                                <h4 class="card-title"><span class="text-primary">{{$milestone->name}}</span> Task Edit</h4>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="{{route('milestone.task.update',[$milestone->id])}}" method="POST">
                                @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group" data-date="23/11/2018" data-date-format="mm/dd/yyyy">
                                        <input type="text" class="form-control range-from" name="start_date" value="{{$task->start_date}}">
                                        <span class="input-group-addon">To</span>
                                        <input class="form-control range-to" type="text" name="finish_date" value="{{$task->completion_date}}">
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="js-basic-single form-control" name="developer">
                                        @foreach($employees as $employee)
                                            <option value="{{$employee->id}}" @if($task->developer_id == $employee->id) selected @endif>{{$employee->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
